...
When you build a spend, the transaction **references an anchor** — a recent, network-recognized root — to say "I am proving membership against *this* snapshot of history." A Sapling Spend "specifies an anchor, which refers to the output Sapling treestate of a previous block" (spec §3.6); Orchard has its `anchorOrchard` field. If the anchor you reference is one the network no longer recognizes (e.g. after a reorg), your spend is invalid — a fact that will matter enormously in Chapter 7.
...
Here is the brutal part, and the reason mobile Zcash is hard: **a witness goes stale as the tree grows.** Every new commitment appended anywhere can change a sibling on your path. An out-of-date witness produces a proof against an anchor the network no longer accepts, and the spend fails — not subtly, but entirely. So the wallet must keep every unspent note's witness **current** as new blocks arrive, which means it must process *every* block's commitments, not just its own. (Chapter 7 is largely about surviving this.)
...
**Every block appends commitments to the tree, and every append can stale your witness — so the wallet must process every block, in order, even blocks with none of its money.** A block with 500 strangers' outputs still grows the tree by 500 leaves, some of which are siblings on your path. Skip it and your witness is silently wrong; the *next* spend fails at broadcast with a stale anchor. There is no "catch up later" — a skipped block is a corrupted witness.

This is why "boring" blocks aren't optional, and it's the single biggest reason a full restore is slow. It is also *universal*: the requirement comes from how spends reference anchors, not from any one engine. Zcash's reference stack implements the tree with the **`shardtree`** crate — "a space-efficient Merkle tree with witnessing of marked leaves, checkpointing & state restoration," represented as "an ordered collection of fixed-depth subtrees, or 'shards'." The server can supply coarse **subtree roots** (`GetSubtreeRoots`) so a wallet doesn't rebuild ancient interior nodes by hand.
...
- **Anchor.** A root (snapshot) of a note commitment tree at some height, which a shielded spend references to prove membership. A stale anchor makes a spend invalid. *(Ch. 3.4, 7.4)*
